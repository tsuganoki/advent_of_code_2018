...
def make_dict_list():
	input_dict = {"+": 0, '-':0}
	input_list = []
	for item in open('001_inputs.txt'):
		item = item.rstrip('')
...
input_dict,input_list =  make_dict_list()
end_val = get_end_val()
...
def find_repeat_freq(input_list):
	# print(input_list)
	freq = 0
	freqs = set([0])
	for change in yield_change(input_list):
		# print(freq, change)
		freq += change
		if freq in freqs:
			return freq
		if len(freqs) > 1000000:
			break
		freqs.add(freq)

# print(find_repeat_freq(input_list))



test = [3, 3, 4, -2, -4, 3, 3, +4, -2, -4]

num = (find_repeat_freq(input_list))
print( "num is", num)